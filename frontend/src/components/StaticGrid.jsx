...
const StaticGrid = forwardRef(
  (
    ...
    useEffect(() => {
      ...
      if (
        JSON.stringify(data) !== JSON.stringify(gridData) &&
        newRows.length === 0
      ) {
        setGridData(data);
      }
    ...
    const handleAddRow = () => {
      ...
      columns.forEach((col) => {
        if (col.key !== "actions") {
          ...
          emptyRow[col.key] =
            newRowDefaults[col.key] !== undefined
    ...
    const handleChange = (rowId, field, value) => {
      ...
      if (onFieldChange) {
        const additionalChanges = onFieldChange(
          rowIdStr,
          field,
          value,
          isNewRow,
      ...
      if (isNewRow) {
        setNewRows(
          newRows.map((r) => (r._id === rowIdStr ? { ...r, ...changes } : r))
        );
      ...
      switch (column.type) {
        case "select":
          ...
          let options =
            typeof column.options === "function"
              ? column.options(
                  isNewRow ? row : { ...row, ...editingRows[rowId] }
                )
              : column.options;

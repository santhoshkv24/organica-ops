...
export const AuthProvider = ({ children }) => {
  ...
  // Inventory loan permission helpers
  console.log('DEBUG AuthContext - user:', user);
  console.log('DEBUG AuthContext - user?.employee:', user?.employee);
  console.log('DEBUG AuthContext - user?.employee?.team_id:', user?.employee?.team_id);
  console.log('DEBUG AuthContext - user?.employee?.is_team_lead:', user?.employee?.is_team_lead);
  
  // Check multiple possible structures for team_id and is_team_lead
  const teamId = user?.employee?.team_id || user?.team_id || null;
  const isEmployeeTeamLead = user?.employee?.is_team_lead == 1 || user?.is_team_lead == 1|| false;
  
  console.log('DEBUG AuthContext - teamId (resolved):', teamId);
  console.log('DEBUG AuthContext - isEmployeeTeamLead (resolved):', isEmployeeTeamLead);
  
  const isHardwareTeamMember = teamId === 5;
  const isHardwareTeamLead = teamId === 5 && isEmployeeTeamLead === true;
  const canApproveInventoryLoans = isHardwareTeamLead;
  const canIssueInventoryLoans = isHardwareTeamMember;
  const canViewAllInventoryLoans = isHardwareTeamMember;
  const canViewTeamInventoryLoans = isHardwareTeamMember;
  
  console.log('DEBUG AuthContext - isHardwareTeamMember:', isHardwareTeamMember);
  console.log('DEBUG AuthContext - isHardwareTeamLead:', isHardwareTeamLead);
  console.log('DEBUG AuthContext - canApproveInventoryLoans:', canApproveInventoryLoans);
  console.log('DEBUG AuthContext - canViewAllInventoryLoans:', canViewAllInventoryLoans);
  console.log('DEBUG AuthContext - canViewTeamInventoryLoans:', isHardwareTeamMember);
  ...
  const value = {
    ...
    // Inventory loan permissions
    isHardwareTeamMember,
    isHardwareTeamLead,
    canApproveInventoryLoans,
    canIssueInventoryLoans,
    canViewAllInventoryLoans,
    canViewTeamInventoryLoans,

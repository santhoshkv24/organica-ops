...
import { getEmployees, getTeams, getCompanies, createEmployee, updateEmployee, deleteEmployee } from '../../services/api';
...
const EmployeeDetails = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [employeesResponse, teamsResponse, companiesResponse] = await Promise.all([
        getEmployees(),
        getTeams(),
        getCompanies()
      ]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => {
        ...
        if (value) {
          const branch = companies.find(c => (c.branch_id || c.company_id) == value);
    ...
    try {
      ...
      if (updates.updated.length > 0) {
        for (const update of updates.updated) {
          ...
          const originalEmployee = employees.find(emp => emp.employee_id == employeeId);
          if (!originalEmployee) continue;
          
          // Merge the original employee data with updates
          const employeeData = {
            first_name: update.first_name !== undefined ? update.first_name : originalEmployee.first_name,
            last_name: update.last_name !== undefined ? update.last_name : originalEmployee.last_name,
            email: update.email !== undefined ? update.email : originalEmployee.email,
            phone: update.phone !== undefined ? update.phone || '' : originalEmployee.phone || '',
            position: update.position !== undefined ? update.position || '' : originalEmployee.position || '',
            team_id: update.team_id !== undefined ? update.team_id : originalEmployee.team_id,
            hire_date: update.hire_date !== undefined ? formatDateForBackend(update.hire_date) : formatDateForBackend(originalEmployee.hire_date)
          };
  ...
  if (searchFilters.team_id) {
    filteredEmployees = filteredEmployees.filter(emp => 
      emp.team_id == searchFilters.team_id
    );
  }

...
export const updatePatchMovementRequestStatus = (patchId, status, approved_by) => handleResponse(() => api.put(`/patch-movements/${patchId}/status`, { status, approved_by }));
export const getPatchMovementRequestByTeamLeadId = (teamLeadId) => handleResponse(() => api.get(`/patch-movements/team-lead/${teamLeadId}`));
export const getPatchMovementRequestsByUser = (userId) => handleResponse(() => api.get(userId ? `/patch-movements/user/${userId}` : '/patch-movements/user'));
...
// Inventory Loan API calls
export const createInventoryLoan = (data) => handleResponse(() => api.post('/inventory-loans', data));
export const approveRejectInventoryLoan = (loanId, status) => handleResponse(() => api.put(`/inventory-loans/${loanId}/approve`, { status }));
export const issueInventoryLoan = (loanId, data) => handleResponse(() => api.put(`/inventory-loans/${loanId}/issue`, data));
export const returnInventoryLoanItems = (loanId, data) => handleResponse(() => api.put(`/inventory-loans/${loanId}/return`, data));
export const getInventoryLoansByUser = (userId) => handleResponse(() => api.get(userId ? `/inventory-loans/user/${userId}` : '/inventory-loans/user'));
export const getInventoryLoanDetails = (loanId) => handleResponse(() => api.get(`/inventory-loans/${loanId}`));
export const getInventoryLoansByProject = (projectId) => handleResponse(() => api.get(`/inventory-loans/project/${projectId}`));
export const getInventoryLoansPendingApproval = (teamId) => handleResponse(() => api.get(`/inventory-loans/team/${teamId}/pending`));
export const getInventoryLoansByTeam = (teamId) => handleResponse(() => api.get(`/inventory-loans/team/${teamId}`));